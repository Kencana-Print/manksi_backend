...
const getSpkInfo = async (req, res) => {
  try {
    ...
    if (!spkNomor) return res.status(400).json({ message: 'SPK nomor wajib diisi' });
...
const getAksesorisByKode = async (req, res) => {
  try {
    ...
    const data = await svc.getAksesorisByKode(kode, parseFloat(spkJumlah), excludeMkaNomor);
    if (!data) return res.status(404).json({ message: 'Kode bahan tidak ditemukan' });
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
const saveData = async (req, res) => {
  try {
    const userKode = req.user?.kduser || '';
...
module.exports = { getDetail, getSpkInfo, getAksesorisMaster, getAksesorisByKode, saveData, deleteData };