const realisasiBarangService = require("../../services/garmen/realisasiBarangService");
...
const getBrowse = async (req, res) => {
  try {
    ...
    const data = await realisasiBarangService.getBrowseData(
      startDate,
      endDate,
      cabang,
      jenis,
...
const deleteData = async (req, res) => {
  try {
    ...
    await realisasiBarangService.deleteData(nomor, req.user.cabang);
...
const requestEdit = async (req, res) => {
  try {
    ...
    await realisasiBarangService.ajukanPerubahan(req.body, req.user);
    res
      .status(200)
