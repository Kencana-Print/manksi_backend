const koreksiStokBarangService = require("../../services/garmen/koreksiStokBarangService");
...
const getBrowse = async (req, res) => {
  try {
    ...
    const data = await koreksiStokBarangService.getBrowseData(
      startDate,
      endDate,
      cabang,
      jenis,
...
const deleteData = async (req, res) => {
  try {
    ...
    await koreksiStokBarangService.deleteData(nomor, req.user);
...
const requestEdit = async (req, res) => {
  try {
    ...
    await koreksiStokBarangService.ajukanPerubahan(req.body, req.user);
    res
      .status(200)
