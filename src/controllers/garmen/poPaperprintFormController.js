const formService = require("../../services/garmen/poPaperprintFormService");

const getMeta = async (req, res) => {
  try {
    const [ukuran, bahan] = await Promise.all([
      formService.getUkuranOptions(),
      formService.getBahanOptions(),
    ]);
    res.status(200).json({
      success: true,
      data: {
        cabangOptions: formService.getCabangOptions(req.user),
        ukuranOptions: ukuran,
        bahanOptions: bahan,
        // finishing: free-text, tidak ada options
      },
    });
  } catch (error) {
...
// req.body.data adalah JSON string (dikirim multipart), req.files dari multer
