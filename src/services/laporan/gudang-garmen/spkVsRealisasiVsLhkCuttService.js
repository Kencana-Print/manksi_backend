...
// ⚠️ FIX: dispatcher sebelumnya TIDAK meneruskan `namaBahan` ke
// getBrowseMap/getBrowseSpk — filter bahan di Browse jadi diam-diam
// tidak pernah diterapkan. Diperbaiki di sini.
const getBrowse = async (
  ...
  isMap
    ? getBrowseMap(startDate, endDate, spk, canLihatCus, namaBahan)
    : getBrowseSpk(startDate, endDate, spk, canLihatCus, namaBahan);

// ─────────────────────────────────────────────
// DETAIL (single SPK) — dipakai UI expand-row, TIDAK diubah.
// Query per satu SPK cukup ringan untuk interaktif.
// ─────────────────────────────────────────────
const getDetail = async (spk, canLihatCus = false) => {
  ...
  const sql = `
    SELECT
      ...
      IFNULL(mlhk.berat, 0) AS BeratLhk,
      ${supplierCol}
      y.Mkb AS NoMkb,
...
// ─────────────────────────────────────────────
// HELPER: subquery header terfilter (kriteria SAMA persis dengan
// getBrowseSpk/getBrowseMap) — dipakai sebagai "hdr" dalam query
// detail gabungan. Karena satu SPK/MAP = satu baris (spk_nomor
// unik), JOIN ke sini otomatis mencegah SPK muncul dobel — beda
// dari versi lama yang loop per-item hasil getBrowse() di JS.
// ─────────────────────────────────────────────
const buildHeaderFilterSpk = (
  startDate,
  endDate,
  spkPrefix,
  canLihatCus,
...
// ─────────────────────────────────────────────
// ALL DETAIL — SATU QUERY GABUNGAN, bukan loop per-SPK. Ini kunci
// perbaikan performa (dari ribuan round-trip jadi 1 query) DAN
// perbaikan duplikasi (hdr subquery = 1 baris per SPK, jadi detail
// tidak bisa "digandakan" walau ada anomali di sisi JS).
// BeratLhk (mlhk) SENGAJA di-drop di sini — tidak dipakai di kolom
// export manapun (beda dari getDetail() interaktif yang butuh
// BeratLhk untuk subtotal di layar).
// ─────────────────────────────────────────────
const buildCombinedDetailSql = (hdrSql, extraSelectDummy = true) => `
  SELECT
    hdr.Divisi, hdr.Nama, hdr.Customer,
    x.Spk,
    x.NoMinta,
    DATE_FORMAT(x.promin_tanggal, '%Y-%m-%d') AS TglMinta,
    x.Gudang,
  ...
  INNER JOIN (
    ...
    LEFT JOIN tgudangproduksi p ON p.gdgp_kode = h.promin_gdgp_kode
    LEFT JOIN tbahan b ON b.Bhn_kode = d.promind_bhn_kode
    GROUP BY h.promin_spk_nomor, h.promin_nomor, d.promind_bhn_kode
  ) x ON x.Spk = hdr.Spk
  LEFT JOIN (
    SELECT
      CAST(GROUP_CONCAT(v.nomor SEPARATOR ', ') AS CHAR) AS nomor,
      v.tgl, v.kode, SUM(v.jml) AS jml, SUM(v.berat) AS berat, v.sat, v.mat
    FROM (
      SELECT m.mph_nomor AS nomor, m.mph_tanggal AS tgl, m.mph_jumlah AS jml,
        m.mph_qty_berat AS berat, m.mph_sat_berat AS sat,
  ...
  ORDER BY hdr.Spk, x.NoMinta, x.promind_bhn_kode
`;
...
const getAllDetail = async (
  ...
  const { sql: hdrSql, params: hdrParams } = isMap
    ? buildHeaderFilterMap(startDate, endDate, spk, canLihatCus, namaBahan)
    : buildHeaderFilterSpk(startDate, endDate, spk, canLihatCus, namaBahan);
  const sql = buildCombinedDetailSql(hdrSql);
  const [rows] = await db.query(sql, hdrParams);
  // Supplier disembunyikan kalau canLihatCus false — hdrSql tidak
  // include supplier (itu dari x.Sup_nama di detail-nya sendiri),
  // jadi masking dilakukan di sini, konsisten dgn getDetail().
  if (!canLihatCus) {
