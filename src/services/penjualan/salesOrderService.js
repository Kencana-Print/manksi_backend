  ...
  const query = `
    ...
    FROM (
      SELECT 
        ...
        s.spk_prasj AS Prasj, s.date_create AS Created, s.spk_jumlah AS Pesan,
        s.spk_jumlah_kirim AS Kirim, (s.spk_jumlah - s.spk_jumlah_kirim) AS Kurang,
      ...
      LEFT JOIN (SELECT lcd_spk_nomor, SUM(IFNULL(lcd_qty_Cetak,0)) AS lcd_qty_Cetak, MIN(lch_tanggal) AS lch_tanggal FROM tlhk_cetakmmt_dtl INNER JOIN tlhk_cetakmmt_hdr ON (lch_nomor=lcd_lch_nomor) GROUP BY 1) l ON l.lcd_spk_nomor = s.spk_nomor
    ) x
    ${whereClause}
...
const deleteOrder = async (nomor, userDetails) => {
  const [rows] = await db.query(
    `SELECT spk_tanggal, spk_divisi, spk_mppb, spk_jumlah_kirim FROM tspk WHERE spk_nomor = ?`,
    [nomor],
  );
...
const requestPin = async (nomor, alasan, userKode) => {
  ...
  const [lastPin] = await db.query(
    `SELECT pin_urut, pin_dipakai FROM tspk_pin5 WHERE pin_trs="SPK" AND pin_nomor=? ORDER BY pin_urut DESC LIMIT 1`,
    [nomor],
  );
