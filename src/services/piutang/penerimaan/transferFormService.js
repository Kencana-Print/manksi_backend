...
const checkPinStatus = async (nomor, conn) => {
  const qPin = `SELECT pin_urut, pin_acc, pin_dipakai FROM tspk_pin5 WHERE pin_trs="PENERIMAAN TRANSFER" AND pin_nomor=? ORDER BY pin_urut DESC LIMIT 1`;
...
const generateNomor = async (cabang, tanggal, conn) => {
  // Format Delphi: SM/BT/202600001
  const tahun = new Date(tanggal).getFullYear().toString();
  const kodeAwal = `${cabang}/BT`;
  ...
  try {
    ...
    if (isEdit) {
      ...
      if (pinInfo.status === "ACC") {
        await conn.query(
          `UPDATE tspk_pin5 SET pin_dipakai = "Y" WHERE pin_trs = "PENERIMAAN TRANSFER" AND pin_nomor = ? AND pin_urut = ?`,
          [nomor, pinInfo.urut],
        );
      }
    } else {
