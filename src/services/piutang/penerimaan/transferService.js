  ...
  const sql = `
    SELECT 
      ...
      IFNULL((
        ...
        "")
        FROM tspk_pin5 
        WHERE pin_trs="PENERIMAAN TRANSFER" AND pin_nomor=t.Nomor 
        ORDER BY pin_urut DESC LIMIT 1
      ), "") AS Ngedit
    FROM terima_bayar_debet t
    LEFT JOIN tkodebayar b ON b.kb_kode = t.kode
    WHERE t.kode LIKE "%BT%" 
      AND t.Tanggal >= ? 
      AND t.Tanggal <= ?
...
// --- 2. DELETE TRANSFER ---
const deleteTransfer = async (nomor) => {
  const [rows] = await db.query(
    `SELECT Tanggal FROM terima_bayar_debet WHERE Nomor = ?`,
    [nomor],
  );
  if (rows.length === 0) throw new Error("Data Transfer tidak ditemukan.");
...
const checkKelayakanPengajuan = async (nomor) => {
  ...
  const tglCloseManual = await tutupBukuService.getManualTutupBuku(
    "PENERIMAAN TRANSFER",
  );
...
const requestPin5 = async (nomor, alasan, userKode) => {
  ...
  const [pinRows] = await db.query(
    `SELECT pin_urut, pin_dipakai FROM tspk_pin5 WHERE pin_trs = "PENERIMAAN TRANSFER" AND pin_nomor = ? ORDER BY pin_urut DESC LIMIT 1`,
    [nomor],
  );
